// todo: fix screenshot and ram free
#include <algorithm>
#include <cassert>
#include <cstdint>
#include <cstdio>
...
static const CalcModel KNOWN_MODELS[] = {
    {0xE001, "TI-GraphLink USB", "cable", 250},  {0xE003, "TI-84 Plus", "z80", 250},
    {0xE004, "TI-89 Titanium", "68k", 1018},     {0xE008, "TI-84 Plus CE", "ez80", 1018},
    {0xE012, "TI-Nspire", "nspire", 1018},       {0xE019, "TI-Nspire CX", "nspire", 1018},
    {0xE022, "TI-Nspire CX II", "nspire", 1018}, {0xE023, "TI-Nspire CX II CAS", "nspire", 1018},
    {0xE025, "TI-84 Plus CE-T", "ez80", 1018},   {0, nullptr, nullptr, 0},
};
...
class TICalc {
  public:
    ...
    void bulk_write(const std::vector<uint8_t>& data) {
        int transferred = 0;
        int r = libusb_bulk_transfer(dev, ep_out, const_cast<uint8_t*>(data.data()),
    ...
    void ping(bool normal_mode = true) {
        negotiate_buffer();
        std::vector<uint8_t> d = {0x00, 0x03, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x07, 0xD0};
        if (!normal_mode)
            d[1] = 0x02;
    ...
    std::vector<std::pair<uint16_t, ParamVal>> parse_params(const std::vector<uint8_t>& blob) {
        ...
        for (int i = 0; i < n && off + 2 <= blob.size(); i++) {
            uint16_t id = be16(blob.data() + off);
            off += 2;
            if (off >= blob.size())
                break;
            ...
            if (pv.valid && off + 2 <= blob.size()) {
                uint16_t len = be16(blob.data() + off);
                off += 2;
    ...
    std::vector<VarEntry> list_vars() {
        ...
        while (true) {
            ...
            uint16_t nattr = be16(vd.data() + off);
            off += 2;
            for (int a = 0; a < nattr && off + 2 <= vd.size(); a++) {
                uint16_t attr_id = be16(vd.data() + off);
                off += 2;
                if (off >= vd.size())
                    break;
                ...
                if (off + 2 > vd.size())
                    break;
                uint16_t attr_len = be16(vd.data() + off);
                off += 2;
                if (off + attr_len > vd.size())
                    break;
                std::vector<uint8_t> adat(vd.begin() + off, vd.begin() + off + attr_len);
                off += attr_len;
                if (attr_id == 0x0001 && adat.size() >= 4)
                    e.size = be32(adat.data());
                if (attr_id == 0x0002 && adat.size() >= 4)
                    e.type_id = be32(adat.data());
                if (attr_id == 0x0003 && adat.size() >= 1)
                    e.archived = (adat[0] != 0);
            }
            vars.push_back(e);
        }
    ...
    void send_var(const std::string& name, uint32_t type_id, const std::vector<uint8_t>& var_data) {
    ...
    void delete_var(const std::string& name, uint32_t type_id) {
        std::vector<uint8_t> d;
        put_be16_append(d, (uint16_t)name.size());
        ...
        put_be16_append(d, 0x0002);
        put_be16_append(d, 4);
        put_be32_append(d, type_id);
        send_vpkt(VPKT_DELETE, d);
        auto [vt, vd] = recv_vpkt();
        if (vt == VPKT_DELAY_ACK)
            recv_vpkt();
    }
    ...
    std::vector<uint8_t> get_screenshot() {
        std::vector<uint8_t> d;
        put_be16_append(d, 1);
        put_be16_append(d, (uint16_t)PID_LCD_CONTENTS);
        send_vpkt(VPKT_PARAM_REQ, d);

        auto [vt, vd] = recv_vpkt();
        if (vt != VPKT_PARAM_DATA)
            throw std::runtime_error("Screenshot: expected PARAM_DATA");

        if (vd.size() < 9)
            throw std::runtime_error("Screenshot: PARAM_DATA too short");

        uint16_t datalen = be16(vd.data() + 5);

        if (7 + (size_t)datalen > vd.size())
            throw std::runtime_error("Screenshot: pixel data truncated");

        return std::vector<uint8_t>(vd.begin() + 7, vd.begin() + 7 + datalen);
    }
...
std::vector<TIVar> parse_ti_file(const std::string& path) {
    ...
    while (off + 4 <= 55 + (size_t)data_size && off + 4 <= buf.size()) {
        uint16_t data_len = (uint16_t)buf[off] | ((uint16_t)buf[off + 1] << 8);
        off += 2;
        uint8_t type_byte = buf[off++];
        uint8_t name_bytes[8];
        memset(name_bytes, 0, 8);
        for (int i = 0; i < 8 && off < buf.size(); i++)
            name_bytes[i] = buf[off++];
        std::string name;
        for (int i = 0; i < 8 && name_bytes[i]; i++)
            name += (char)name_bytes[i];
        if (off + data_len > buf.size())
            break;
        TIVar v;
        v.name = name;
...
void write_ti_file(const std::string& path, const TIVar& var) {
    ...
    for (int i = 0; i < 42; i++)
        out.push_back((uint8_t)comment[i]);
...
static void print_usage() {
    fprintf(stderr,
            ...
            "  --disable-test           Disable test mode (sends dummy variable)\n"
            //"  --screenshot             Capture LCD screenshot (saved to screenshot.bin or --out
            //path)\n"
            "\n"
            "Examples:\n"
            "  ticli --list\n"
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-d") {
            ...
            if (++i >= argc) {
                fprintf(stderr, "-t requires milliseconds\n");
                return 1;
            }
            ...
            if (!action.empty()) {
                fprintf(stderr,
                        "Error: multiple action flags specified (%s and %s). "
    ...
    try {
...

        {
            ...
            if (action == "--info") {
                calc.ping();
                static const uint16_t INFO_PIDS[] = {PID_PRODUCT_NAME, PID_OS_VERSION, 0x000D,
                                                     PID_RAM_FREE,     PID_FLASH_FREE, PID_BATTERY};
                auto blob = calc.request_params(std::vector<uint16_t>(INFO_PIDS, INFO_PIDS + 6));
                ...
                if (vars.empty()) {
                    ...
                    printf("%s  %s  %s  %s\n", rpad("Name", 12).c_str(), lpad("Type", 16).c_str(),
                           lpad("Size", 8).c_str(), "Archived");
                    printf("%s  %s  %s  %s\n", rpad("----", 12).c_str(), lpad("----", 16).c_str(),
                           lpad("----", 8).c_str(), "--------");
                    for (auto& v : vars) {
                        uint8_t tb = type_id_to_byte(v.type_id);
                        printf("%s  %s  %s  %s\n", rpad(v.name, 12).c_str(),
                               lpad(type_name(tb), 16).c_str(),
                               lpad(std::to_string(v.size), 8).c_str(), v.archived ? "Yes" : "No");
                    }
                }
                ...
                if (action_args.empty()) {
                    ...
                    for (auto& path : action_args) {
                        ...
                        for (auto& v : vars) {
                            printf("  Sending %s (%s, %zu bytes) ...", v.name.c_str(),
                                   type_name(v.type_byte), v.data.size());
                            fflush(stdout);
                            calc.send_var(v.name, v.type_id, v.data);
                            printf(" OK\n");
                        }
                    }
                }
                ...
                if (action_args.empty()) {
                    ...
                    calc.ping();
                    printf("Receiving %s ...", varname.c_str());
                    fflush(stdout);
                    auto data = calc.recv_var(varname, type_byte_to_id(0x05));
                    printf(" %zu bytes\n", data.size());

                    TIVar v;
                    v.name = varname;
                    v.type_byte = 0x05;
                    v.type_id = type_byte_to_id(0x05);
                    v.data = data;
                ...
                if (action_args.empty()) {
                    ...
                    for (auto& c : varname)
                        c = toupper(c);
                    calc.ping();
                    printf("Deleting %s ...", varname.c_str());
                    fflush(stdout);
                    calc.delete_var(varname, type_byte_to_id(0x05));
                    printf(" OK\n");
                }
                ...
                calc.send_var("EXAMRST", type_byte_to_id(0x15), dummy_body);
                printf("Transfer complete. Cleaning up...\n");
                calc.delete_var("EXAMRST", type_byte_to_id(0x15));
                printf("Done. Calculator should have exited exam mode.\n");
            }
            /* else if (action == "--screenshot") {
                std::string save_path = out_path.empty() ? "screenshot.bin" : out_path;
                calc.ping();
                printf("Capturing LCD contents...\n");
                auto data = calc.get_screenshot();
                printf("  %zu bytes received\n", data.size());
                std::ofstream f(save_path, std::ios::binary);
                if (!f) throw std::runtime_error("Cannot write " + save_path);
                f.write((char*)data.data(), data.size());
                printf("Saved raw bitmap to %s\n", save_path.c_str());
                if (data.size() == 768)
                    printf("(96x64 monochrome, 1 bit per pixel, MSB first)\n");
                else if (data.size() == 153600)
                    printf("(320x240 RGB565, 2 bytes per pixel, big-endian)\n");
                else
                    printf("(%zu bytes, unknown format)\n", data.size());
            }  this is broken too*/
